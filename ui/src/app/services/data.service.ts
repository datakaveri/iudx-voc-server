...
import { ClassDetail, Class, Classes, Property, Properties } from '../types/classDetail';
      ...
      map( resp => {
        ...
        for (var node of resp["@graph"]) {
          ...
          if (node["@type"].includes("rdfs:Class")) {
            if (nodeId == className) {
              flattened.baseClass = {label: className,
                comment: nodeComment,
            ...
            for (var domain of node["iudx:domainIncludes"]) {
              if (domain["@id"] == "iudx:"+className){
                flattened.baseClass.properties.push(<Property>{label: nodeId,
                                                    comment: nodeComment,
                                                    type: ranges});
              }
            }
